...
resource "vault_policy" "db_read_policy" {
  depends_on = [vault_kv_secret_v2.database]

  name = "vault-db-reader"
  policy = <<EOT
    path "${vault_kv_secret_v2.database.path}" {
      capabilities = ["read"]
    }
  EOT
}
...
resource "vault_kubernetes_auth_backend_role" "service_reader_role" {
  ...
  backend                          = vault_auth_backend.kubernetes.path
  bound_service_account_names      = [kubernetes_service_account.vault_db_reader.metadata.0.name]
  bound_service_account_namespaces = ["httpd"]
